...
ResourceType ResourceId::type() const {
    ResourceType type = static_cast<ResourceType>((id >> (
        ResourceIdLimits::ID_BITS + ResourceIdLimits::GENERATION_BITS)) &
        (ResourceIdLimits::RESOURCE_TYPES_COUNT - 1));
    return type;
}

uint32_t ResourceId::index() const {
    return static_cast<uint32_t>((id & ~ResourceIdLimits::GENERATION_MASK) & 
        (ResourceIdLimits::PARENT_IDS_COUNT - 1));
}

uint32_t ResourceId::generation() const {
    return static_cast<uint32_t>(((id & ResourceIdLimits::GENERATION_MASK) >> ResourceIdLimits::ID_BITS) & 
        (ResourceIdLimits::GENERATION_COUNT - 1));
}
